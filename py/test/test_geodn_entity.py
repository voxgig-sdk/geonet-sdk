...
class TestGeodnEntity:

    def test_should_create_instance(self):
        testsdk = GeonetSDK.test(None, None)
        ent = testsdk.Geodn(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _geodn_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "geodn." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GEONET_TEST_GEODN_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        geodn_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.geodn")))
        geodn_ref01_data = None
        if len(geodn_ref01_data_raw) > 0:
            geodn_ref01_data = helpers.to_map(geodn_ref01_data_raw[0][1])

        # LOAD
        geodn_ref01_ent = client.Geodn(None)
        geodn_ref01_match_dt0 = {}
        geodn_ref01_data_dt0_loaded, err = geodn_ref01_ent.load(geodn_ref01_match_dt0, None)
        assert err is None
        assert geodn_ref01_data_dt0_loaded is not None



def _geodn_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/geodn/GeodnTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GEONET_TEST_GEODN_ENTID"))
