...
class GeonetTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GeonetTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GeonetTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GeonetTestRunner.getenv("GEONET_TEST_LIVE")
        override = GeonetTestRunner.getenv("GEONET_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GeonetTestRunner.getenv(key)
        ...
        explain = GeonetTestRunner.getenv("GEONET_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GEONET_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GeonetTestRunner._test_control is not None:
            return GeonetTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GeonetTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GeonetTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GeonetTestRunner.load_test_control()
...
def load_env_local():
    GeonetTestRunner.load_env_local()


def env_override(m):
    return GeonetTestRunner.env_override(m)


def entity_data(v):
    return GeonetTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GeonetTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GeonetTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GeonetTestRunner.load_test_control()


def live_delay_ms():
    return GeonetTestRunner.live_delay_ms()
