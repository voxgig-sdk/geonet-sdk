...
class TestPingDirect:

    def test_should_direct_load_ping(self):
        setup = _ping_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ping", "live" if setup["live"] else "unit")
        if _skip:
...
def _ping_direct_setup(mockres):
