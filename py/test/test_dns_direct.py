...
class TestDnsDirect:
    ...
    def test_should_direct_load_dns(self):
        setup = _dns_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dns", "live" if setup["live"] else "unit")
        if _skip:
