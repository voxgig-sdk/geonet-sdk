...
class TestGeodnDirect:

    def test_should_direct_load_geodn(self):
        setup = _geodn_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-geodn", "live" if setup["live"] else "unit")
        if _skip:
...
def _geodn_direct_setup(mockres):
