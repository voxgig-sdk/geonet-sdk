...
class PingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GEONET_TEST_PING_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    ping_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.ping")))
    ping_ref01_data = nil
    if ping_ref01_data_raw.length > 0
      ping_ref01_data = Helpers.to_map(ping_ref01_data_raw[0][1])
    end

    # LOAD
    ping_ref01_ent = client.Ping(nil)
    ping_ref01_match_dt0 = {}
    ping_ref01_data_dt0_loaded, err = ping_ref01_ent.load(ping_ref01_match_dt0, nil)
    assert_nil err
...
def ping_basic_setup(extra)
  ...
  entid_env_raw = ENV["GEONET_TEST_PING_ENTID"]
