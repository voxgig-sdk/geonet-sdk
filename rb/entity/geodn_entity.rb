...
class GeodnEntity
  ...
  def make
    opts = @_entopts.dup
    GeodnEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GeonetHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Geodn, Hash] the current Geodn data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GeonetHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Geodn fields)
  def match_get
  ...
  # Load a single Geodn.
  #
  # @param reqmatch [GeodnLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Geodn.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Geodn, Hash] the loaded Geodn; raises GeonetError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GeonetHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
