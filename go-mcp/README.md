...
MCP server exposing the Geonet SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `geonet_list` | `entity`, optional `query` map | First page of records as JSON |
| `geonet_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `geonet_list` and
`geonet_load` tools become available in new sessions.
