...
[MCP](https://modelcontextprotocol.io) server exposing the Geonet SDK as
two agent tools — `geonet_list` and `geonet_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geonet-mcp)
make build

# 2. Provide credentials via the environment
export GEONET_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user geonet \
  -- /absolute/path/to/geonet-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./geonet-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// geonet_list: first page of records
{ "entity": "dns" }
{ "entity": "dns", "query": { } }

// geonet_load: one record by id
{ "entity": "dns", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geonet-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user geonet \
     -- "$PWD"/dist/*/geonet-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `geonet_list` and `geonet_load` tools now appear
   in new sessions. Ask the agent to *"list dns using geonet"*
   and it calls `geonet_list` with `{"entity":"dns"}`.
...
```sh
export GEONET_APIKEY=sk_live_xxx            # API key
export GEONET_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `geonet_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `geonet_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GEONET_APIKEY` | API key sent with every request. |
| `GEONET_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

dns | geodn | geoping | ping

### Smoke test via HTTP (raw JSON-RPC)

```sh
./geonet-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"geonet_load","arguments":{"entity":"dns","query":{"id":1}}}}'
```
