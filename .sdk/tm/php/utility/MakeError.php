...
// Geonet SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class GeonetMakeError
{
    public static function call(?GeonetContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GeonetContext([], null);
        }
        $op = $ctx->op ?? new GeonetOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GeonetResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GeonetError) ? $err->msg : (string)$err;
        $msg = "GeonetSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new GeonetError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof GeonetError) {
