...
module GeonetUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = GeonetContext.new({}, nil)
    end
    op = ctx.op || GeonetOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || GeonetResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(GeonetError) ? err.msg : err.to_s
    ...
    sdk_err = GeonetError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(GeonetError)
