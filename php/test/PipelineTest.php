...
{
    private static function utility(): GeonetUtility
    {
        return new GeonetUtility();
    }
    ...
    private static function ctx(array $over = []): GeonetContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GeonetContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GeonetOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GeonetError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GeonetOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GeonetMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GeonetOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GeonetMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GeonetOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GeonetMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeonetSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GeonetMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GeonetResponse([]),
            'result' => new GeonetResult([]),
        ]);
        [, $err] = GeonetMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GeonetSpec(['step' => 's']),
            'response' => new GeonetResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GeonetResult([]),
        ]);
        [, $err] = GeonetMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GeonetSpec(['step' => 's']),
            'response' => new GeonetResponse(self::resp(200, ['v' => 1])),
            'result' => new GeonetResult([]),
        ]);
        [, $err] = GeonetMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GeonetSpec(['step' => 's']),
            'response' => new GeonetResponse(self::resp(200, ['v' => 2])),
            'result' => new GeonetResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GeonetMakeResponse::call($ctx);
    ...
    {
        $preset = new GeonetResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GeonetSpec([]),
            'response' => new GeonetResponse([]),
            'result' => new GeonetResult([]),
        ...
        [$got, $err] = GeonetMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GeonetResult([])]);
        [, $err] = GeonetMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GeonetSpec([]), 'result' => null]);
        [, $err] = GeonetMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeonetSpec(['step' => 's']),
            'result' => new GeonetResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GeonetOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeonetMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeonetSpec(['step' => 's']),
            'result' => new GeonetResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GeonetOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeonetMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeonetResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GeonetSpec([]),
            'result' => new GeonetResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GeonetMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeonetMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GeonetError('boom', 'boom');
        $utility->fetcher = function (GeonetContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeonetSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GeonetMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GeonetContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GeonetMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GeonetResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GeonetContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeonetSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GeonetMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GeonetResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GeonetSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GeonetMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GeonetMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GeonetMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GeonetResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GeonetDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GeonetResult(['ok' => false])]);
        $this->expectException(GeonetError::class);
        GeonetDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GeonetResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GeonetMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GeonetBaseFeature();
        $b = new GeonetBaseFeature();
        GeonetFeatureAdd::call($ctx, $a);
        GeonetFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GeonetBaseFeature
    {
        $f = new GeonetBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GeonetFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GeonetContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GeonetSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeonetPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GeonetResponse(['headers' => null]),
            'result' => new GeonetResult([]),
        ]);
        GeonetResultHeaders::call($ctx);
