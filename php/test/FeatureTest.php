...
class FtCtrl extends GeonetControl
{
...
class FtClient
{
    ...
    private GeonetUtility $utility;

    public function __construct(GeonetUtility $utility, array $options)
    {
    ...
    public function get_utility(): GeonetUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GeonetContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GeonetContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GeonetConfig::make_feature($name);
    ...
    private static function buildUrl(GeonetSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GeonetError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GeonetResponse($fetched);
        }
    ...
    private function fail(GeonetContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GeonetContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
