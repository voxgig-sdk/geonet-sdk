...
class DnsEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GEONET_TEST_DNS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function dns_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["dns01", "dns02", "dns03"] as $k) {
    ...
    $entid_env_raw = getenv("GEONET_TEST_DNS_ENTID");
