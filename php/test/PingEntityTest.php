...
class PingEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GEONET_TEST_PING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ping_ref01_data = null;
        if (count($ping_ref01_data_raw) > 0) {
...
function ping_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GEONET_TEST_PING_ENTID");
