...
class GeonetRetryFeature extends GeonetBaseFeature
{
    ...
    public function init(GeonetContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GeonetContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GeonetContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GeonetContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
