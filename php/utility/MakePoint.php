...
// Geonet SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class GeonetMakePoint
{
    public static function call(GeonetContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof GeonetError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = GeonetHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = GeonetHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
