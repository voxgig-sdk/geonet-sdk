...
class GeopingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GeopingEntity($this->_client, $opts);
    }

    /**
     * @param Geoping|array $args Geoping data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GeonetHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Geoping|array The current Geoping data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Geoping fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GeonetHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Geoping fields).
     */
     ...
     *
     * @param GeopingLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GeopingLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Geoping|array The loaded Geoping as an assoc-array at the
     *   SDK boundary; throws GeonetError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GeonetHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
