...
boru-driven command-line client **and** interactive REPL for the Geonet
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geonet-cli)
make build
...
./geonet-cli load 1 dns            # {id:1} shorthand
./geonet-cli load '{id:1}' dns       # explicit match map

# 5. Override the API base URL for a single call
GEONET_BASE=https://api.example.com ./geonet-cli load 1 dns
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geonet-cli
   ```
...
```sh
./geonet-cli load 1 dns          # scalar shorthand for {id:1}
./geonet-cli load '{id:1}' dns     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `geonet>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dns`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/geonet-cli`. |
...
(`./geonet-cli <expr>`) and interactively (the REPL), and expressions compose the
