...
func TestPingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GEONET_TEST_PING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		pingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.ping", setup.data)))
		var pingRef01Data map[string]any
		if len(pingRef01DataRaw) > 0 {
			pingRef01Data = core.ToMapAny(pingRef01DataRaw[0][1])
		}
...
func pingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GEONET_TEST_PING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GEONET_TEST_PING_ENTID"])
