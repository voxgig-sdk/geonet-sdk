...
func TestDnsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GEONET_TEST_DNS_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		dnsRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.dns", setup.data)))
		var dnsRef01Data map[string]any
		if len(dnsRef01DataRaw) > 0 {
			dnsRef01Data = core.ToMapAny(dnsRef01DataRaw[0][1])
		}
...
func dnsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GEONET_TEST_DNS_ENTID")
