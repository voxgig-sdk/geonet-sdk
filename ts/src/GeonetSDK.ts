...
class GeonetSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('GeonetSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('GeonetSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('GeonetSDK: graphql: ' +
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Geodn(entopts?: Record<string, any>) {
    const self = this
    return new GeodnEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Geoping(entopts?: Record<string, any>) {
    const self = this
    return new GeopingEntity(self, entopts)
  }
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new GeonetSDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return GeonetSDK.test(testopts, sdkopts)
  }
  ...
  toString() {
    return 'Geonet ' + this._utility.struct.jsonify(this.toJSON())
  }
