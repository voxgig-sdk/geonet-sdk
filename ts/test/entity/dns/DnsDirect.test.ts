...
describe('DnsDirect', async () => {
  ...
  test('direct-load-dns', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-dns', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GEONET_TEST_DNS_ENTID']
