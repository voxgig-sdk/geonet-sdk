...
//     (\`new GeonetSDK(...)\` / \`GeonetSDK.test(...)\` become
//     \`GeonetSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"dns":{"test01":{"id":"test01"}},"geodn":{"test01":{"id":"test01"}},"geoping":{"test01":{"id":"test01"}},"ping":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GeonetSDK(...)\` and any
// \`GeonetSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GeonetSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GeonetSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
