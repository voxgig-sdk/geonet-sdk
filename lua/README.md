...
The Lua SDK for the Geonet API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Dns(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Geonet(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── geonet_sdk.lua    -- Main SDK module
...
The main module (`geonet_sdk`) exports the SDK constructor
