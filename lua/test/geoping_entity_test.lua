...
describe("GeopingEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Geoping(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = geoping_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "geoping." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GEONET_TEST_GEOPING_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local geoping_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.geoping")))
    local geoping_ref01_data = nil
    if #geoping_ref01_data_raw > 0 then
      geoping_ref01_data = helpers.to_map(geoping_ref01_data_raw[1][2])
    end
...
function geoping_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/geoping/GeopingTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read geoping test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GEONET_TEST_GEOPING_ENTID")
