...
describe("PingEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GEONET_TEST_PING_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local ping_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.ping")))
    local ping_ref01_data = nil
    if #ping_ref01_data_raw > 0 then
      ping_ref01_data = helpers.to_map(ping_ref01_data_raw[1][2])
    end
...
function ping_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GEONET_TEST_PING_ENTID")
