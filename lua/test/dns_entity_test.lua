...
describe("DnsEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GEONET_TEST_DNS_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local dns_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.dns")))
    local dns_ref01_data = nil
    if #dns_ref01_data_raw > 0 then
      dns_ref01_data = helpers.to_map(dns_ref01_data_raw[1][2])
    end
...
function dns_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GEONET_TEST_DNS_ENTID")
