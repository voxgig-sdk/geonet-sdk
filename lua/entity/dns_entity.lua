...
local DnsEntity = {}
DnsEntity.__index = DnsEntity


function DnsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DnsEntity)
  self._name = "dns"
  self._client = client
...
function DnsEntity:get_name()
  return self._name
end


function DnsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DnsEntity.new(self._client, opts)
end
...
function DnsEntity:mark_deleted()
  self._deleted = true
end


function DnsEntity:deleted()
  return true == self._deleted
end


function DnsEntity:data_set(args)
  if args ~= nil then
...
function DnsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DnsEntity:match_set(args)
  if args ~= nil then
...
function DnsEntity:stream(action, args, callopts)
...
function DnsEntity:_run_op(ctx, post_done)
  local utility = self._utility
