...
local GeopingEntity = {}
GeopingEntity.__index = GeopingEntity


function GeopingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeopingEntity)
  self._name = "geoping"
  self._client = client
...
function GeopingEntity:get_name()
  return self._name
end


function GeopingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeopingEntity.new(self._client, opts)
end
...
function GeopingEntity:mark_deleted()
  self._deleted = true
end


function GeopingEntity:deleted()
  return true == self._deleted
end


function GeopingEntity:data_set(args)
  if args ~= nil then
...
function GeopingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeopingEntity:match_set(args)
  if args ~= nil then
...
function GeopingEntity:match_get()
...
function GeopingEntity:stream(action, args, callopts)
...
function GeopingEntity:_run_op(ctx, post_done)
  local utility = self._utility
