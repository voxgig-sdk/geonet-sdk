...
local GeodnEntity = {}
GeodnEntity.__index = GeodnEntity


function GeodnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeodnEntity)
  self._name = "geodn"
  self._client = client
...
function GeodnEntity:get_name()
  return self._name
end


function GeodnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeodnEntity.new(self._client, opts)
end
...
function GeodnEntity:mark_deleted()
  self._deleted = true
end


function GeodnEntity:deleted()
  return true == self._deleted
end


function GeodnEntity:data_set(args)
  if args ~= nil then
...
function GeodnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeodnEntity:match_set(args)
  if args ~= nil then
...
function GeodnEntity:match_get()
...
function GeodnEntity:stream(action, args, callopts)
...
function GeodnEntity:_run_op(ctx, post_done)
  local utility = self._utility
